    ...
    stages {
        stage('rocketmq-cluster start'){
            steps {
                ...
                sh 'sleep 10'
                sh 'docker exec -ti rmqbroker sh mqadmin updateTopic -n namesrv:9876 -b localhost:10911 -t broadcastTest'
                sh 'docker exec -ti rmqbroker sh mqadmin updateTopic -n namesrv:9876 -b localhost:10911 -t sendAndReceive'
                sh 'docker exec -ti rmqbroker sh mqadmin updateTopic -n namesrv:9876 -b localhost:10911 -t sendOnewayAndReceive'
                sh 'docker exec -ti rmqbroker sh mqadmin updateTopic -n namesrv:9876 -b localhost:10911 -t rebalance'
                sh 'docker exec -ti rmqbroker sh mqadmin updateTopic -n namesrv:9876 -b localhost:10911 -t tagTest'
            }
        }
        stage('CentOS 6'){
            agent {
